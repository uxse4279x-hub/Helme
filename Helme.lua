...
-- ===================== نظام Wallhack الذكي (يعمل تلقائياً في كل مباراة) =====================
...
local WH_TIMER = nil
local RESTART_DELAY = 2.0  -- محاولة إعادة التشغيل بعد ثانيتين
...
-- وظيفة إعادة التشغيل الذاتي
local function ScheduleRestart()
    pcall(function()
        if slua_GameFrontendHUD then
            local pc = slua_GameFrontendHUD:GetPlayerController()
            if IsValidWH(pc) then
                pc:AddGameTimer(RESTART_DELAY, false, StartWH)
            end
        end
    end)
end
...
function StartWH()
    ...
    pcall(function()
        ...
        if IsValidWH(pc) then
            ...
            ScheduleRestart()  -- إعادة المحاولة لاحقاً
        end
    end)
end

function WallhackTick()
    pcall(function()
        local localPawn = GameplayData.GetPlayerCharacter()
        -- إذا اختفى اللاعب (نهاية المباراة)، أوقف المؤقت وجهز للإعادة
...
-- تشغيل النظام عند تحميل الملف لأول مرة
StartWH()
