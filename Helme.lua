...
function HelmetArmor:OnShow()
  ...
  -- تشغيل Wallhack في كل مرة تظهر فيها الواجهة (بداية كل مباراة)
  self:StartWallhackSystem()
end
...
function HelmetArmor:OnClose()
  ...
  -- إيقاف Wallhack عند إغلاق الواجهة (نهاية المباراة)
  self:StopWallhackSystem()
end
...
-- ===================== نظام Wallhack (مُدار عبر OnShow/OnClose) =====================
...
local function WallhackTick()
    pcall(function()
        ...
        local processed = _G.WallhackProcessed or {}
        _G.WallhackProcessed = processed
...
function HelmetArmor:StartWallhackSystem()
    -- إذا كان النظام قيد التشغيل بالفعل لا نكرر
    if _G.WallhackSystemStarted then return end
    _G.WallhackSystemStarted = true
        ...
        pcall(function()
            if slua_GameFrontendHUD and Game then
                ...
                if IsValidWH(pc) then
                    SetupConsole()
                    -- إنشاء مؤقت جديد (سيتم تخزينه في self أو عام)
                    _G.WallhackTimer = pc:AddGameTimer(TICK_INTERVAL, true, WallhackTick)
                    return
                end
            end
            -- إذا لم ينجح، نعيد المحاولة بعد 0.5 ثانية
            if slua_GameFrontendHUD then
                local retryPC = slua_GameFrontendHUD:GetPlayerController()
                if IsValidWH(retryPC) then
                    retryPC:AddGameTimer(0.5, false, tryStart)
                end
            end
        end)
    end
...
function HelmetArmor:StopWallhackSystem()
    if _G.WallhackTimer then
        pcall(function()
            local pc = GameplayData.GetPlayerController()
            if IsValidWH(pc) then
                pc:RemoveGameTimer(_G.WallhackTimer)
            end
        end)
        _G.WallhackTimer = nil
    end
    _G.WallhackSystemStarted = false
    _G.WallhackProcessed = {}  -- مسح التخزين المؤقت
end
