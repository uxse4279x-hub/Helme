...
-- ===================== نظام Wallhack (فقط) =====================
-- ===================== نظام Wallhack (محدث ليعمل بكل مباراة) =====================
...
local function WallhackTick()
    pcall(function()
        -- تنظيف الجدول دورياً لتجنب تراكم البيانات القديمة
...
local function StartWallhackSystem()
    -- إعادة تعيين الذاكرة المؤقتة عند بدء محاولة جديدة لضمان عمله في كل مباراة
    _G.WallhackProcessed = {}
        ...
        pcall(function()
            if slua_GameFrontendHUD and Game then
                ...
                if IsValidWH(pc) then
                    SetupConsole()
                    -- إلغاء التايمر القديم إن وجد وإنشاء جديد لتفادي التداخل
                    if WH_TIMER then
                        WH_TIMER = nil
                    end
            ...
            -- إعادة المحاولة إذا لم يتم تجهيز الـ HUD بعد
...
-- تشغيل النظام بدون شروط قاطعة تمنعه من التكرار
StartWallhackSystem()
