...
-- ===================== واجهة الترحيب التلقائية (تم تطوير هذا بواسطة بابلو) =====================
local function ShowAndHideEmptyMenu()
    local Z_MENU = 11000
    
    local function GetScaleFactor()
        local height = 1080
        pcall(function() height = slua_GameFrontendHUD:GetWorld():GetResolutionY() end)
        if height <= 480 then return 0.444
        elseif height <= 720 then return 0.444 + (height - 480) * (0.666 - 0.444) / (720 - 480)
        elseif height <= 1080 then return 0.666 + (height - 720) * (1.0 - 0.666) / (1080 - 720)
        else return 1.0 + (height - 1080) * (8.0 - 1.0) / (8640 - 1080)
        end
    end

    local panel = import("CanvasPanel")()
    if not panel or not slua.isValid(panel) then return end

    require("game_frontend_hud").AddToContainer(UIContainers.Top, panel, Z_MENU)

    local panelSlot = import("WidgetLayoutLibrary").SlotAsCanvasSlot(panel)
    panelSlot:SetAnchors(FAnchors(0.5, 0.5, 0.5, 0.5))
    panelSlot:SetAlignment(FVector2D(0.5, 0.5))
    local scale = GetScaleFactor()
    panelSlot:SetSize(FVector2D(350 * scale, 100 * scale))
    panelSlot:SetPosition(FVector2D(0, 0))

    local bgImage = import("Image")()
    bgImage:SetColorAndOpacity(FLinearColor(0, 0, 0, 0.85))
    panel:AddChild(bgImage)
    local bgSlot = import("WidgetLayoutLibrary").SlotAsCanvasSlot(bgImage)
    bgSlot:SetAnchors(FAnchors(0, 0, 1, 1))
    bgSlot:SetOffsets(FMargin(0, 0, 0, 0))

    local txt = import("TextBlock")()
    if txt and slua.isValid(txt) then
        txt:SetText("تم تطوير هذا بواسطة بابلو")
        txt:SetColorAndOpacity(FLinearColor(1, 0.84, 0, 1)) -- أصفر ذهبي
        panel:AddChild(txt)
        local txtSlot = import("WidgetLayoutLibrary").SlotAsCanvasSlot(txt)
        if txtSlot then
            txtSlot:SetAnchors(FAnchors(0, 0, 1, 1))
            txtSlot:SetOffsets(FMargin(0, 0, 0, 0))
        end
    end

    -- إخفاء وتدمير اللوحة تلقائياً بعد 5 ثوانٍ
    later(5.0, function()
...
-- استدعاء واجهة "تم تطوير هذا بواسطة بابلو" تلقائياً بعد دخول الماتش
later(2.0, function()
    ShowAndHideEmptyMenu()
end)
