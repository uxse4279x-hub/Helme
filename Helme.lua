...
-- ===================== نظام Wallhack الذكي (يعمل في كل المباريات) =====================
...
local function WallhackTick()
    pcall(function()
        ...
        if not IsValidWH(localPawn) then
            -- إذا اختفى اللاعب (مات أو انتهت المباراة)، نوقف المؤقت ونعيد التشغيل لاحقاً
            StopWallhackTimer()
            return
        end
...
function StopWallhackTimer()
    if WH_TIMER then
        pcall(function()
            local pc = GameplayData.GetPlayerController()
            if IsValidWH(pc) then
                pc:RemoveGameTimer(WH_TIMER)
            end
        end)
        WH_TIMER = nil
    end
...
function StartWallhackTimer()
    if WH_TIMER then return end  -- يعمل بالفعل
    local function tryStart()
        pcall(function()
            if slua_GameFrontendHUD and Game then
                ...
                if IsValidWH(pc) then
                    SetupConsole()
                    _G.WallhackProcessed = {}  -- مسح التخزين للمباراة الجديدة
                    WH_TIMER = pc:AddGameTimer(TICK_INTERVAL, true, WallhackTick)
                    return
                end
            end
            -- المحاولة مرة أخرى بعد 0.5 ثانية
            if slua_GameFrontendHUD then
                ...
                if IsValidWH(retryPC) then
                    ...
                    -- المحاولة الأخيرة عبر مؤقت عالمي
                    if _G.SetTimer then
...
-- تشغيل Wallhack تلقائياً عند تحميل الملف لأول مرة
StartWallhackTimer()

-- ===================== وظائف HelmetArmor الأصلية =====================
