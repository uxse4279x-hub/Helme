...
-- ===================== نظام ESP مع شريط الصحة (تمت إضافته فقط) =====================
-- إعدادات ESP
local CFG = {
    ESP = {
        MaxDist = 300,
...
local function _M_StickmanESP()
    pcall(function()
        ...
        if not slua.isValid(CachedHUD) then
            local pc = slua_GameFrontendHUD:GetPlayerController() or GameplayStatics.GetPlayerController(slua_GameFrontendHUD:GetWorld(), 0)
            if slua.isValid(pc) then CachedHUD = pc:GetHUD() end
        end
